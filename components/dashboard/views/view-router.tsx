...
const CauHinhDatBanView = dynamic(() =>
  import("./reservation/cau-hinh-dat-ban-view").then((m) => m.CauHinhDatBanView)
);
const KhachHangThanhVienView = dynamic(() =>
  import("./customers/khach-hang-thanh-vien-view").then((m) => m.KhachHangThanhVienView)
);
const NhaCungCapView = dynamic(() =>
  import("./suppliers/nha-cung-cap-view").then((m) => m.NhaCungCapView)
);
const BaoCaoView = dynamic(() => import("./reports/bao-cao-view").then((m) => m.BaoCaoView));
...
export function ViewRouter({
  ...
  switch (currentView) {
    case "cau-hinh-dat-ban":
      return <CauHinhDatBanView activeTab={activeTab} setActiveTab={setActiveTab} />;
